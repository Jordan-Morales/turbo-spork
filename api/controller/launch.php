...
if ($_REQUEST['action'] === 'index') {
  ...
  $new_launch = new launch(null, $body_object->flight_number, $body_object->launch_date_local, $body_object->rocket);
  $all_launches = Launches::create($new_launch);
  echo json_encode($all_launches);
} else if ($_REQUEST['action'] === 'update'){
  $request_body = file_get_contents('php://input');
  $body_object = json_decode($request_body);
  $updated_launch = new launch($_REQUEST['id'], $body_object->like);
